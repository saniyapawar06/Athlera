...
# Milestone catalog built purely on real sporting metrics (no XP / currency).
ACHIEVEMENTS: list[dict[str, Any]] = [
    # milestones
    {"code": "first_win",   "title": "First Win",           "desc": "Win your first ranked match.",              "tier": "bronze",   "category": "milestone",   "icon": "flash",            "scope": "global"},
    {"code": "upset_win",   "title": "Giant Slayer",        "desc": "Beat a higher-rated opponent.",             "tier": "gold",     "category": "milestone",   "icon": "shield-checkmark", "scope": "global"},
    {"code": "new_pb",      "title": "New Peak",            "desc": "Set a new personal-best rating.",           "tier": "silver",   "category": "milestone",   "icon": "trending-up",      "scope": "sport"},
    # streaks
    {"code": "streak_3",    "title": "On a Roll",           "desc": "Win 3 matches in a row.",                   "tier": "silver",   "category": "streak",      "icon": "flame",            "scope": "sport", "threshold": 3},
    {"code": "streak_5",    "title": "Red Hot",             "desc": "Win 5 matches in a row.",                   "tier": "gold",     "category": "streak",      "icon": "flame",            "scope": "sport", "threshold": 5},
    {"code": "streak_10",   "title": "Unstoppable",         "desc": "Win 10 matches in a row.",                  "tier": "platinum", "category": "streak",      "icon": "flame",            "scope": "sport", "threshold": 10},
    # matches played
    {"code": "matches_10",  "title": "Contender",           "desc": "Play 10 matches.",                          "tier": "bronze",   "category": "milestone",   "icon": "tennisball",       "scope": "sport", "threshold": 10},
    {"code": "matches_25",  "title": "Regular",             "desc": "Play 25 matches.",                          "tier": "silver",   "category": "milestone",   "icon": "tennisball",       "scope": "sport", "threshold": 25},
    {"code": "matches_50",  "title": "Veteran",             "desc": "Play 50 matches.",                          "tier": "gold",     "category": "milestone",   "icon": "tennisball",       "scope": "sport", "threshold": 50},
    {"code": "matches_100", "title": "Centurion",           "desc": "Play 100 matches.",                         "tier": "platinum", "category": "milestone",   "icon": "tennisball",       "scope": "sport", "threshold": 100},
    # ranking
    {"code": "top_100",     "title": "Top 100",             "desc": "Break into the top 100.",                   "tier": "bronze",   "category": "rank",        "icon": "podium",           "scope": "sport", "threshold": 100},
    {"code": "top_50",      "title": "Top 50",              "desc": "Break into the top 50.",                    "tier": "silver",   "category": "rank",        "icon": "podium",           "scope": "sport", "threshold": 50},
    {"code": "top_10",      "title": "Top 10",              "desc": "Break into the top 10.",                    "tier": "gold",     "category": "rank",        "icon": "podium",           "scope": "sport", "threshold": 10},
    {"code": "top_3",       "title": "Podium",              "desc": "Reach the top 3.",                          "tier": "platinum", "category": "rank",        "icon": "medal",            "scope": "sport", "threshold": 3},
    {"code": "rank_1",      "title": "Number One",          "desc": "Reach #1 in the rankings.",                 "tier": "diamond",  "category": "rank",        "icon": "trophy",           "scope": "sport", "threshold": 1},
    # competitions
    {"code": "league_champion",     "title": "League Champion",     "desc": "Win a league.",        "tier": "gold",     "category": "competition", "icon": "trophy", "scope": "global"},
    {"code": "knockout_champion",   "title": "Knockout Champion",   "desc": "Win a knockout.",      "tier": "gold",     "category": "competition", "icon": "trophy", "scope": "global"},
    {"code": "tournament_champion", "title": "Tournament Champion", "desc": "Win a tournament.",    "tier": "platinum", "category": "competition", "icon": "trophy", "scope": "global"},
    # multi-sport
    {"code": "multi_2",     "title": "Dual Threat",         "desc": "Get rated in 2 sports.",                    "tier": "bronze",   "category": "multi",       "icon": "sparkles",         "scope": "global", "threshold": 2},
    {"code": "multi_3",     "title": "Triple Threat",       "desc": "Get rated in 3 sports.",                    "tier": "silver",   "category": "multi",       "icon": "sparkles",         "scope": "global", "threshold": 3},
    {"code": "multi_5",     "title": "Complete Athlete",    "desc": "Compete across all 5 sports.",              "tier": "platinum", "category": "multi",       "icon": "star",             "scope": "global", "threshold": 5},
]
ACH_BY_CODE = {a["code"]: a for a in ACHIEVEMENTS}


def _ach_public(a: dict) -> dict:
    return {
        "code": a["code"], "title": a["title"], "desc": a.get("desc"),
        "tier": a["tier"], "category": a["category"], "icon": a["icon"],
        "scope": a.get("scope", "global"),
        "sport_id": a.get("sport_id"),
        "unlocked_at": a["unlocked_at"].isoformat() if isinstance(a.get("unlocked_at"), datetime) else a.get("unlocked_at"),
    }


async def _unlock(user_id: str, code: str, sport_id: Optional[str], meta: Optional[dict] = None) -> Optional[dict]:
    """Idempotently unlock an achievement. Returns the doc only if newly unlocked."""
    spec = ACH_BY_CODE.get(code)
    if not spec:
        return None
    existing = await DB.achievements.find_one(
        {"user_id": user_id, "code": code, "sport_id": sport_id}, {"_id": 0}
    )
    if existing:
        return None
    doc = {
        "id": str(uuid.uuid4()), "user_id": user_id, "code": code, "sport_id": sport_id,
        "title": spec["title"], "desc": spec["desc"], "tier": spec["tier"],
        "category": spec["category"], "icon": spec["icon"], "scope": spec.get("scope", "global"),
        "meta": meta or {}, "unlocked_at": now_utc(),
    }
    await DB.achievements.insert_one(doc)
    return {k: v for k, v in doc.items() if k != "_id"}


async def _evaluate_achievements(user_id: str, sport_id: str, ctx: dict) -> list[dict]:
    """Check & unlock all metric-based achievements after a match. Returns newly unlocked (public)."""
    ps = await _get_ps(user_id, sport_id)
    if not ps:
        return []
    newly: list[dict] = []

    async def add(code: str, sid: Optional[str], meta: Optional[dict] = None):
        a = await _unlock(user_id, code, sid, meta)
        if a:
            newly.append(_ach_public(a))

    is_winner = bool(ctx.get("is_winner"))
    if is_winner:
        await add("first_win", None)
        if ctx.get("upset"):
            await add("upset_win", None)

    if ctx.get("rc", {}).get("new_peak"):
        await add("new_pb", sport_id)

    cur_streak = ps.get("current_streak", 0)
    for code in ("streak_3", "streak_5", "streak_10"):
        if cur_streak >= ACH_BY_CODE[code]["threshold"]:
            await add(code, sport_id)

    mp = ps.get("matches_played", 0)
    for code in ("matches_10", "matches_25", "matches_50", "matches_100"):
        if mp >= ACH_BY_CODE[code]["threshold"]:
            await add(code, sport_id)

    rank, total = await _rank_for_user(user_id, sport_id, ps["rating"])
    for code in ("top_100", "top_50", "top_10", "top_3"):
        if rank <= ACH_BY_CODE[code]["threshold"]:
            await add(code, sport_id, {"rank": rank})
    if rank == 1:
        await add("rank_1", sport_id, {"rank": 1})

    sports_count = await DB.player_sports.count_documents({"user_id": user_id, "matches_played": {"$gt": 0}})
    for code in ("multi_2", "multi_3", "multi_5"):
        if sports_count >= ACH_BY_CODE[code]["threshold"]:
            await add(code, None)

    return newly


async def _store_ranks_and_achievements(match_id: str, sport_id: str, rating_changes: dict, upset_winner: bool) -> None:
    """Run after all player_sports are updated: store weekly rank + evaluate achievements."""
    for uid, rc in rating_changes.items():
        rank, _ = await _rank_for_user(uid, sport_id, rc["after"])
        await DB.rating_history.update_one(
            {"match_id": match_id, "user_id": uid}, {"$set": {"rank": rank}}
        )
        newly = await _evaluate_achievements(uid, sport_id, {
            "is_winner": rc["is_winner"], "rc": rc,
            "upset": rc["is_winner"] and upset_winner,
        })
        rc["achievements"] = newly
    await DB.matches.update_one({"id": match_id}, {"$set": {"rating_changes": rating_changes}})
...
def _knockout_round_label(round_no: int, total_rounds: Optional[int]) -> str:
    if not total_rounds:
        return f"Round {round_no}"
    remaining = total_rounds - round_no
    return {0: "Final", 1: "Semi-Final", 2: "Quarter-Final", 3: "Round of 16", 4: "Round of 32"}.get(
        remaining, f"Round {round_no}"
    )


async def _next_fixture_label(user_id: str, sport_id: str) -> Optional[dict]:
    fixtures = await DB.fixtures.find(
        {"sides.user_ids": user_id, "status": "scheduled"}, {"_id": 0}
    ).sort("round", 1).to_list(50)
    for f in fixtures:
        c = await DB.competitions.find_one({"id": f["competition_id"]}, {"_id": 0})
        if not c or c.get("sport_id") != sport_id or c.get("status") != "in_progress":
            continue
        if c["type"] == "knockout":
            label = _knockout_round_label(f.get("round", 1), f.get("total_rounds"))
        else:
            label = "League match"
        return {"label": label, "competition": c.get("name")}
    return None


def _fmt_gap(value: float, decimals: int) -> str:
...
async def _build_sport_insights(user_id: str, card: dict) -> list[dict]:
    ...
    peak = card.get("peak_rating", rating)
    ...
    if not provisional and mp > 0:
        if peak > rating:
            out.append({"icon": "trending-up", "tone": "pb",
                        "text": f"{_fmt_gap(peak - rating, dec)} points to Personal Best"})
        elif mp > 1 and rating >= peak:
    ...
    if rank:
        delta = await _rank_delta_week(user_id, sid, rank)
        if delta > 0:
            out.append({"icon": "arrow-up", "tone": "up", "text": f"{delta} place{'s' if delta != 1 else ''} this week"})
        elif delta < 0:
            out.append({"icon": "arrow-down", "tone": "warn", "text": f"{-delta} place{'s' if delta != -1 else ''} this week"})

    nf = await _next_fixture_label(user_id, sid)
    if nf:
        out.append({"icon": "git-network-outline", "tone": "comp", "text": f"{nf['label']} next"})

    return out


async def _award_competition_achievements(cid: str) -> None:
    c = await DB.competitions.find_one({"id": cid}, {"_id": 0})
    if not c:
        return
    ctype = c.get("type")
    if ctype == "knockout":
        for uid in (c.get("champion_ids") or []):
            await _unlock(uid, "knockout_champion", None, {"competition_id": cid, "name": c.get("name")})
        return
    # league / tournament: award when every fixture is resolved
    pending = await DB.fixtures.count_documents(
        {"competition_id": cid, "status": {"$nin": ["complete", "bye"]}}
    )
    total_fix = await DB.fixtures.count_documents({"competition_id": cid})
    if total_fix == 0 or pending > 0:
        return
    standings = await _compute_standings(cid)
    if not standings:
        return
    champ = standings[0]["user_id"]
    code = "tournament_champion" if ctype == "tournament" else "league_champion"
    await _unlock(champ, code, None, {"competition_id": cid, "name": c.get("name")})
    await DB.competitions.update_one(
        {"id": cid}, {"$set": {"status": "complete", "champion_ids": [champ]}}
    )


async def _backfill_achievements(user_id: str) -> None:
    """One-time reconstruction of milestones from a user's existing history."""
    user = await DB.users.find_one({"id": user_id}, {"_id": 0, "ach_backfilled": 1})
    if not user or user.get("ach_backfilled"):
        return
    ...
    for ps in ps_list:
        ...
        if mp > 0:
            sports_with_play += 1
        if ps.get("wins", 0) >= 1:
            await _unlock(user_id, "first_win", None)
        best = ps.get("best_streak", 0)
        for code in ("streak_3", "streak_5", "streak_10"):
            if best >= ACH_BY_CODE[code]["threshold"]:
                await _unlock(user_id, code, sid)
        for code in ("matches_10", "matches_25", "matches_50", "matches_100"):
            if mp >= ACH_BY_CODE[code]["threshold"]:
                await _unlock(user_id, code, sid)
        # new PB — genuine peak above starting rating
        first_hist = await DB.rating_history.find_one(
            {"user_id": user_id, "sport_id": sid}, {"_id": 0, "before": 1}, sort=[("created_at", 1)]
        )
        if first_hist and ps.get("peak_rating", 0) > first_hist.get("before", 1e9):
            await _unlock(user_id, "new_pb", sid)
        # ranking
        rank, _ = await _rank_for_user(user_id, sid, ps["rating"])
        for code in ("top_100", "top_50", "top_10", "top_3"):
            if rank <= ACH_BY_CODE[code]["threshold"]:
                await _unlock(user_id, code, sid, {"rank": rank})
        if rank == 1:
            await _unlock(user_id, "rank_1", sid, {"rank": 1})

    # upset — any verified win as the lower-rated player
    won_matches = await DB.matches.find(
    ...
    for m in won_matches:
        ...
        if me and opp and me.get("before", 0) < opp.get("before", 0):
            await _unlock(user_id, "upset_win", None)
            break

    for code in ("multi_2", "multi_3", "multi_5"):
        if sports_with_play >= ACH_BY_CODE[code]["threshold"]:
            await _unlock(user_id, code, None)

    # competitions won
    knock = await DB.competitions.find({"type": "knockout", "champion_ids": user_id}, {"_id": 0, "id": 1, "name": 1}).to_list(50)
    for c in knock:
        await _unlock(user_id, "knockout_champion", None, {"competition_id": c["id"], "name": c.get("name")})
    my_members = await DB.competition_members.find(
        {"user_id": user_id, "status": "approved"}, {"_id": 0, "competition_id": 1}
    ).to_list(200)
    for mem in my_members:
        c = await DB.competitions.find_one({"id": mem["competition_id"]}, {"_id": 0})
        if not c or c.get("type") not in ("league", "tournament"):
            continue
        pending = await DB.fixtures.count_documents(
            {"competition_id": c["id"], "status": {"$nin": ["complete", "bye"]}}
        )
        total_fix = await DB.fixtures.count_documents({"competition_id": c["id"]})
        if total_fix == 0 or pending > 0:
            continue
        standings = await _compute_standings(c["id"])
        if standings and standings[0]["user_id"] == user_id:
            code = "tournament_champion" if c["type"] == "tournament" else "league_champion"
            await _unlock(user_id, code, None, {"competition_id": c["id"], "name": c.get("name")})

    await DB.users.update_one({"id": user_id}, {"$set": {"ach_backfilled": True}})


@api.get("/me/achievements")
async def my_achievements(u: dict = Depends(current_user)):
    await _backfill_achievements(u["id"])
    unlocked = await DB.achievements.find({"user_id": u["id"]}, {"_id": 0}).sort("unlocked_at", -1).to_list(500)
    ...
    for spec in ACHIEVEMENTS:
        catalog.append({
            ...
            "unlocked": spec["code"] in unlocked_codes,
        })
    out_unlocked = []
    for a in unlocked:
        sport_name = SPORT_BY_ID[a["sport_id"]]["name"] if a.get("sport_id") in SPORT_BY_ID else None
        out_unlocked.append({**_ach_public(a), "sport_name": sport_name, "meta": a.get("meta", {})})
    return {
        "unlocked": out_unlocked,
        "catalog": catalog,
...
async def social_share(body: ShareIn, u: dict = Depends(current_user)):
    doc = {
        ...
        "kind": body.kind, "headline": body.headline, "subtext": body.subtext,
        "icon": body.icon or "megaphone", "sport_id": body.sport_id,
        "achievement_code": body.achievement_code, "created_at": now_utc(),
    }
...
async def dashboard(u: dict = Depends(current_user)):
    ...
    recent_ach = await DB.achievements.find({"user_id": u["id"]}, {"_id": 0}).sort("unlocked_at", -1).limit(8).to_list(8)
    recent_achievements = [
        {**_ach_public(a), "sport_name": SPORT_BY_ID[a["sport_id"]]["name"] if a.get("sport_id") in SPORT_BY_ID else None}
        for a in recent_ach
    ]
    ach_docs = await DB.achievements.find({"user_id": u["id"]}, {"_id": 0, "code": 1}).to_list(500)
    ach_total = len({d["code"] for d in ach_docs})
    return {
        "user": public_user(u),
        "uas": uas_info["uas"],
...
async def match_submit(body: MatchSubmitIn, u: dict = Depends(current_user)):
    ...
    if doc["status"] == "verified":
        ...
        return {"match": applied, "rating_change": rc, "my_id": u["id"]}
    return {"match": {k: v for k, v in doc.items() if k != "_id"}, "rating_change": None, "my_id": u["id"]}
...
async def _apply_verified_match(match_id: str) -> None:
    ...
    await _store_ranks_and_achievements(match_id, sport_id, rating_changes, w_ps["rating"] < l_ps["rating"])
...
async def social_feed(u: dict = Depends(current_user)):
    # Merge user-shared highlights (PBs, streaks, rank jumps, achievements, wins)
    # with a lightweight stream of recent verified match results.
    ...
    items = []
    for s in shares:
        author = umap.get(s["user_id"], {}).get("display_name", "Athlete")
        items.append({
            "id": s["id"], "type": "highlight", "kind": s.get("kind"),
            "sport_id": s.get("sport_id"), "author": author,
            "is_mine": s["user_id"] == u["id"],
            "headline": s.get("headline"), "subtext": s.get("subtext"),
            "icon": s.get("icon", "megaphone"),
            "created_at": iso(s.get("created_at")),
        })
    for m in matches:
        winner = umap.get(m["winner_user_id"], {}).get("display_name", "Athlete")
        others = [p for p in m["participants"] if p != m["winner_user_id"]]
        loser = umap.get(others[0], {}).get("display_name", "Athlete") if others else "Athlete"
        items.append({
...
async def _apply_result_general(match_id: str) -> None:
    ...
    await _store_ranks_and_achievements(match_id, sport_id, rating_changes, win_rating < lose_rating)
    if m.get("fixture_id"):
...
async def live_finalize(live_id: str, u: dict = Depends(current_user)):
    ...
    return {"match_id": match_id, "rating_changes": final.get("rating_changes", {}),
            ...
            "competition_id": lm.get("competition_id"),
...
async def _progress_fixture(fixture_id: str, match_id: str) -> None:
    ...
    await _award_competition_achievements(f["competition_id"])
