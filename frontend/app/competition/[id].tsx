...
export default function CompetitionDetailScreen() {
  ...
  const saveSchedule = async () => {
    const iso = schedAt ? schedAt.toISOString() : null;
    await act(() => api.fixtureUpdate(scheduleFixture.id, { scheduled_at: iso }));
    setScheduleFixture(null); setSchedAt(null);
  };

  const addFixture = async () => {
    if (!pickA || !pickB || pickA === pickB) { setErr("Pick two different players"); return; }
    await act(() => api.compAddFixture(cid, { side0_user_ids: [pickA], side1_user_ids: [pickB], scheduled_at: fixAt ? fixAt.toISOString() : null }));
    ...
    <SafeAreaView style={styles.root} edges={["top"]} testID="competition-detail-screen">
              ...
              isKO ? (
                <View>
                  ...
                  <KnockoutBracket
                    ...
                    memberName={(uid: string) => memberName(c, uid)}
                    accent={accent}
                    championIds={c.champion_ids}
                    isOrganiser={!!c.is_organiser}
                    onPressFixture={(f: any) => setFxActions(f)}
                  />
                </View>
              ) :
              rounds.map((r: any) => {
      ...
      {scheduleFixture && (
        <View style={styles.sheet} testID="schedule-sheet">
          ...
          <DateTimeField value={schedAt} onChange={setSchedAt} accent={accent} testIDPrefix="sched-dt" />
          <View style={{ flexDirection: "row", gap: spacing.sm }}>
            <Pressable testID="sched-clear" onPress={() => setSchedAt(null)} style={styles.ghost}><Text style={styles.ghostText}>UNSCHEDULE</Text></Pressable>
      ...
      {addFixtureOpen && (
        <View style={styles.sheetTall} testID="add-fixture-sheet">
          ...
          <Text style={styles.label}>DATE & TIME (OPTIONAL)</Text>
          <DateTimeField value={fixAt} onChange={setFixAt} accent={accent} testIDPrefix="af-dt" />
      ...
      {/* fixture actions sheet (bracket / league tap) */}
      {fxActions && (
        <View style={styles.sheet} testID="fixture-actions-sheet">
          <Text style={styles.sheetTitle}>{fxActions.round_name || `ROUND ${fxActions.round}`}</Text>
          <Text style={styles.sheetSub}>{memberName(c, fxActions.sides[0].user_ids[0])} vs {fxActions.sides[1].user_ids[0] ? memberName(c, fxActions.sides[1].user_ids[0]) : "TBD"}</Text>
          <Pressable testID="fxa-schedule" onPress={() => { const f = fxActions; setFxActions(null); setSchedAt(f.scheduled_at ? new Date(f.scheduled_at) : null); setScheduleFixture(f); }} style={styles.actionRow}>
            <Ionicons name="calendar" size={20} color={colors.onSurface} /><Text style={styles.actionText}>Edit date / time · Reschedule</Text>
          </Pressable>
          <Pressable testID="fxa-final" onPress={() => { const f = fxActions; setFxActions(null); setG([{ a: "", b: "" }]); setScoringFixture(f); }} style={styles.actionRow}>
            <Ionicons name="create" size={20} color={colors.onSurface} /><Text style={styles.actionText}>Enter Final Score</Text>
          </Pressable>
          <Pressable testID="fxa-live" onPress={() => { const f = fxActions; setFxActions(null); router.push(`/live/setup?sport_id=${c.sport_id}&competition_id=${c.id}&fixture_id=${f.id}`); }} style={[styles.actionRow, { borderColor: colors.brand }]}>
...
const styles = StyleSheet.create({
  ...
  actionRow: { flexDirection: "row", alignItems: "center", gap: spacing.md, padding: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, backgroundColor: colors.surface },
  actionText: { ...font.textBold, fontSize: 14, color: colors.onSurface },
  tapHint: { flexDirection: "row", alignItems: "center", gap: 4 },
  tapHintText: { ...font.textBold, letterSpacing: 1, fontSize: 10, color: colors.onSurfaceSecondary },
