...
const CATEGORIES = ["all", "oneoff", "league", "knockout"];
const CAT_LABEL: Record<string, string> = { all: "ALL", oneoff: "ONE-OFF", league: "LEAGUE", knockout: "KNOCKOUT" };
...
export default function MatchHistoryScreen() {
  ...
  }, [sport, result, category]);
  useEffect(() => { load(); }, [load]);

  const wins = matches.filter((m) => m.won).length;

  const ChipRow = ({ items, value, onChange, tid, labels }: any) => (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow} style={{ height: 44 }}>
      {items.map((it: string) => (
        <Pressable key={it} testID={`${tid}-${it}`} onPress={() => onChange(it)} style={[styles.chip, value === it && { borderColor: colors.brand, backgroundColor: colors.surfaceTertiary }]}>
          <Text style={[styles.chipText, value === it && { color: colors.onSurface }]}>{labels ? labels[it] : it.toUpperCase()}</Text>
        </Pressable>
      ))}
    </ScrollView>
  );
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top"]} testID="match-history-screen">
      <View style={styles.header}>
        ...
        {matches.length > 0 && <Text style={styles.record}>{wins}W · {matches.length - wins}L</Text>}
      </View>
      <View style={styles.filters}>
        <ChipRow items={SPORTS} value={sport} onChange={setSport} tid="mh-sport" />
        <ChipRow items={CATEGORIES} value={category} onChange={setCategory} tid="mh-category" labels={CAT_LABEL} />
            ...
            <Pressable key={m.id} onPress={() => router.push(`/match/${m.id}`)} style={styles.row} testID={`mh-row-${m.id}`}>
              ...
              <View style={{ alignItems: "flex-end" }}>
                ...
                <View style={styles.badges}>
                  <Text style={[styles.comp, m.category === "oneoff" && { color: colors.onSurfaceTertiary }]}>{CAT_LABEL[m.category] || "ONE-OFF"}</Text>
                </View>
              </View>
...
const styles = StyleSheet.create({
  ...
  record: { ...font.textBold, fontSize: 13, color: colors.onSurfaceSecondary, marginLeft: "auto" },
