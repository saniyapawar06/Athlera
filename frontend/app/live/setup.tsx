...
export default function LiveSetupScreen() {
  ...
  const { sport_id, competition_id, fixture_id, opp_id, opp_name } = useLocalSearchParams<{ sport_id: string; competition_id?: string; fixture_id?: string; opp_id?: string; opp_name?: string }>();
  ...
  useEffect(() => {
    if (opp_id && !opponent) {
      setMode("oneoff");
      setOpponent({ user_id: opp_id as string, display_name: (opp_name as string) || "Opponent", rating: "—" });
    }
  }, [opp_id, opp_name]);
