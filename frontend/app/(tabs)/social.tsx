...
export default function SocialScreen() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [n, l, r] = await Promise.all([api.nearby(), api.ltpList(), api.playRequestsMine()]);
      setNearby(n.players || []); setLtp(l.posts || []); setReqs(r);
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top"]} testID="social-screen">
      ...
      <View style={styles.tabs}>
        {(["nearby", "ltp", "requests"] as Tab[]).map((t) => (
          <Pressable key={t} testID={`social-tab-${t}`} onPress={() => setTab(t)} style={[styles.tab, tab === t && { borderBottomColor: colors.brand }]}>
            <Text style={[styles.tabText, tab === t && { color: colors.onSurface }]}>{t === "ltp" ? "LOOKING TO PLAY" : t.toUpperCase()}</Text>
          </Pressable>
        ))}
      </View>
