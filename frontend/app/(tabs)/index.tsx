...
import { toneColor } from "@/src/gamification";
        ...
        {(() => {
          ...
          return (
            <Animated.View entering={FadeInDown.delay(120).duration(500)} testID="gamify-strip">
              ...
              {/* Trophy room link */}
              <Pressable testID="trophy-room-link" onPress={() => router.push("/trophies")} style={styles.trophyLink}>
                <Ionicons name="trophy" size={15} color={colors.brand} />
                <Text style={styles.trophyLinkText}>
                  {(g.achievement_count ?? 0)}/{(g.achievement_total ?? 0)} ACHIEVEMENTS
                </Text>
                <Text style={styles.trophyLinkCta}>TROPHY ROOM →</Text>
              </Pressable>
        ...
        {ALL_SPORT_META.map((meta, i) => {
          ...
          return (
            <Animated.View key={meta.id} entering={FadeInDown.delay(100 + i * 70).duration(400)}>
              ...
              {owned && (owned.insights ?? []).length > 0 && (
                <View style={styles.insightsRow} testID={`insights-${meta.id}`}>
                  {owned.insights.slice(0, 4).map((ins: any, idx: number) => (
...
const styles = StyleSheet.create({
  ...
  insightsRow: { flexDirection: "row", flexWrap: "wrap", gap: 6, marginHorizontal: spacing.md, marginTop: -2, marginBottom: spacing.sm },
  ...
  trophyLink: { flexDirection: "row", alignItems: "center", gap: spacing.sm, marginHorizontal: spacing.md, marginTop: spacing.sm, padding: spacing.md, borderWidth: 1, borderColor: colors.border, borderRadius: radius.md, backgroundColor: colors.surfaceSecondary },
  trophyLinkText: { ...font.textBold, letterSpacing: 1, fontSize: 11, color: colors.onSurface, flex: 1 },
  trophyLinkCta: { ...font.textBold, letterSpacing: 1, fontSize: 10, color: colors.brand },
