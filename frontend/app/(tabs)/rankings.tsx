...
import { colors, spacing, radius, font, sportAccent, formatRating, countryOf } from "@/src/theme";
...
export default function RankingsScreen() {
  ...
  const myCity = (user as any)?.city as string | undefined;
  const myCountry = countryOf(myCity);

  const scopeParams = useCallback((): Record<string, string> => {
    if (scope === "city" && myCity) return { city: myCity };
    if (scope === "country" && myCountry) return { country: myCountry };
    return {};
  }, [scope, myCity, myCountry]);
  ...
  const load = useCallback(async (tab: string) => {
    ...
    try {
      const q = scopeParams();
      if (tab === "uas") {
        const r = await api.rankingsUAS(q);
        setRows(r.rows || []); setSportMeta(null);
      } else {
        const r = await api.rankingsSport(tab, q);
  ...
  const SCOPES: { id: "global" | "country" | "city"; label: string; disabled?: boolean }[] = [
    { id: "global", label: "GLOBAL" },
    { id: "country", label: myCountry ? myCountry.toUpperCase() : "COUNTRY", disabled: !myCountry },
    { id: "city", label: myCity ? myCity.toUpperCase() : "CITY", disabled: !myCity },
  ];
  ...
  return (
    <SafeAreaView style={styles.root} edges={["top"]} testID="rankings-screen">
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Global · Country · City leaderboards</Text>
        ...
        <View style={styles.scopeRow}>
          {SCOPES.map((s) => (
            <Pressable
              key={s.id}
              testID={`ranking-scope-${s.id}`}
              disabled={s.disabled}
              onPress={() => setScope(s.id)}
              style={[styles.scopeChip, scope === s.id && { borderColor: colors.brand, backgroundColor: colors.surfaceTertiary }, s.disabled && { opacity: 0.4 }]}
            >
              <Text style={[styles.scopeText, scope === s.id && { color: colors.onSurface }]} numberOfLines={1}>{s.label}</Text>
            </Pressable>
          ))}
        </View>
      </View>
      <ScrollView contentContainerStyle={styles.scroll}>
        {loading ? <ActivityIndicator color={colors.brand} style={{ marginTop: spacing.lg }} /> :
          rows.length === 0 ? (
            <View style={styles.empty}><Text style={styles.emptyTitle}>No rankings for this scope yet</Text></View>
          ) : rows.map((r) => {
...
const styles = StyleSheet.create({
  ...
  scopeRow: { flexDirection: "row", gap: spacing.sm, marginTop: 4 },
  scopeChip: { flex: 1, height: 34, borderWidth: 1, borderColor: colors.border, borderRadius: radius.pill, alignItems: "center", justifyContent: "center", backgroundColor: colors.surfaceSecondary, paddingHorizontal: 6 },
  scopeText: { ...font.textBold, letterSpacing: 1, fontSize: 9, color: colors.onSurfaceSecondary },
