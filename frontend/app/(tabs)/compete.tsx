...
import { colors, spacing, radius, font, sportAccent, countryOf } from "@/src/theme";
...
export default function CompeteScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (scope === "city" && myCity) q.city = myCity;
      if (scope === "country" && myCountry) q.country = myCountry;
      const [c, e] = await Promise.all([api.compList(), api.events(q)]);
      setComps(c.competitions || []); setEvents(e.events || []);
    ...
    <SafeAreaView style={styles.root} edges={["top"]} testID="compete-screen">
      <View style={styles.stickyHeader}>
        ...
        {tab === "events" && (
          <View style={styles.scopeRow}>
            {([["global", "GLOBAL", false], ["country", myCountry ? myCountry.toUpperCase() : "COUNTRY", !myCountry], ["city", myCity ? myCity.toUpperCase() : "CITY", !myCity]] as [any, string, boolean][]).map(([id, lbl, disabled]) => (
              <Pressable key={id} testID={`event-scope-${id}`} disabled={disabled} onPress={() => setScope(id)} style={[styles.scopeChip, scope === id && { borderColor: colors.brand, backgroundColor: colors.surfaceTertiary }, disabled && { opacity: 0.4 }]}>
                <Text style={[styles.scopeText, scope === id && { color: colors.onSurface }]} numberOfLines={1}>{lbl}</Text>
              </Pressable>
            ))}
          </View>
        )}
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        tab === "competitions" ? (
          ...
          fEvents.length === 0 ? <Empty t="No public events for this filter — tap CREATE EVENT" /> :
          fEvents.map((e, i) => (
...
const styles = StyleSheet.create({
  ...
  scopeChip: { flex: 1, height: 32, borderWidth: 1, borderColor: colors.border, borderRadius: radius.pill, alignItems: "center", justifyContent: "center", backgroundColor: colors.surfaceSecondary, paddingHorizontal: 6 },
